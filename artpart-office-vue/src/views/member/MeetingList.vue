<template>
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        ...
        <h2>대표자 회의</h2>
            ...
            제목<input type="text" style="width: 5%" v-model="search_value">  공지여부<select id="category" v-model="search_value1" style="width: 5%">
        ...
        <div class="table-responsive">
          <table class="table table-striped table-sm">
            ...
            <tbody>
              <tr v-for="(row, meetingidx) in list" :key="meetingidx">
                <td>{{row.meetingid.emp_name}}</td>
                  <td><a v-on:click="fnView(row.meetingidx)">{{row.meetingtitle}}</a></td>
                <td>{{ row.meetingtime }}</td>
                <td>{{row.meetingopen}}</td>
              </tr>
  ...
  export default {
      ...
      methods:{
          fetchMeeting(){
              ...
              this.$axios.get(this.$serverUrl+"/meeting/list",{
