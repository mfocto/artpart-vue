<template>
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
          <h1 class="h2">입주민</h1>
          
        </div>
  
        
  
        
        <div class="table-responsive">
          
          <table class="table table-striped table-sm">
            <thead class="boarder-0">
              <tr>
                <th scope style="text-align:left;">등록제목</th>
                <th scope="col" style="text-align:left;" colspan="3"><input type="text" style="width: 30%" value="공사" /></th>
              </tr>
              <tr>
                <th scope style="text-align:left;">작성자</th>
                <th scope="col" style="text-align:left;" colspan="3"><input type="text" style="width: 30%" value="김송이" ></th>
              </tr>
              <tr>
                <th scope style="text-align:left;">등록날짜</th>
                <th scope="col" style="text-align:left;" colspan="3"><input type="text" style="width: 30%" value="2020-01-01" ></th>
              </tr>
              <tr>
                <th scope style="text-align:left;">마감날짜</th>
                <th scope="col" style="text-align:left;" colspan="3"><input type="text" style="width: 30%" value="2020-01-11" ></th>
              </tr>
              
              <tr>
                <th scope style="text-align: left; vertical-align: top;" >내용</th>
                <td colspan="4" style="text-align:left;"  >
                     <textarea  rows="10" style="width: 30%;">101동 동대표 투표</textarea>
                </td>
              </tr>
          
          
              <tr>
                <th scope style="text-align:left;">답변1</th>
                <th scope="col" style="text-align:left;" colspan="3"><input type="text" style="width: 30%" value="송지형" ></th>
              </tr>
              <tr>
                <th scope style="text-align:left;">답변2</th>
                <th scope="col" style="text-align:left;" colspan="3"><input type="text" style="width: 30%" value="송지형" ></th>
              </tr>
              <tr>
                <th scope style="text-align:left;">답변3</th>
                <th scope="col" style="text-align:left;" colspan="3"><input type="text" style="width: 30%" value="송지형" ></th>
              </tr>
              <tr>
                <th scope style="text-align:left;">답변4</th>
                <th scope="col" style="text-align:left;" colspan="3"><input type="text" style="width: 30%" value="송지형" ></th>
              </tr>
              <tr>
                <th scope style="text-align:left;">답변5</th>
                <th scope="col" style="text-align:left;" colspan="3"><input type="text" style="width: 30%" value="송지형" ></th>
              </tr>
              <tr>
                <th scope style="text-align:left;">답변6</th>
                <th scope="col" style="text-align:left;" colspan="3"><input type="text" style="width: 30%" value="" ></th>
              </tr>
              <tr>
                <th scope style="text-align:left;">답변7</th>
                <th scope="col" style="text-align:left;" colspan="3"><input type="text" style="width: 30%" value="" ></th>
              </tr>
              <tr>
                <th scope style="text-align:left;">답변8</th>
                <th scope="col" style="text-align:left;" colspan="3"><input type="text" style="width: 30%" value="" ></th>
              </tr>
              <tr>
                <th scope style="text-align:left;">답변9</th>
                <th scope="col" style="text-align:left;" colspan="3"><input type="text" style="width: 30%" value="" ></th>
              </tr>
              <tr>
                <th scope style="text-align:left;">답변10</th>
                <th scope="col" style="text-align:left;" colspan="3"><input type="text" style="width: 30%" value="" ></th>
              </tr>
             
              <tr>
                <th scope style="text-align:left;">마감</th>
                <th scope="col" style="text-align:left;" colspan="3">
                    <input type="radio"  name="a" value="" >Y  
                    <input type="radio"  name="a" value="" >N
                </th>
              </tr>
             
              <tr>
                <th scope="col" colspan="4" style="text-align:right;">
                  <button type="button" class="btn btn-sm btn-outline-secondary">목록</button>
                  <button type="button" class="btn btn-sm btn-outline-secondary">작성</button>
                </th>
              </tr>
            </thead>
           
          </table>
        </div>
  </template>
  
  <script>
  export default {
    data() {
    return {
      isReadonly: true,
      selectedOption: 'selected' 
    };
  },
  methods: {
    toggleReadonly() {
      this.isReadonly = !this.isReadonly;
    }
  }

  }
  </script>
  
  <!-- Add "scoped" attribute to limit CSS to this component only -->
  <style scoped>
  
  h3 {
    margin: 40px 0 0;
  }
  ul {
    list-style-type: none;
    padding: 0;
  }
  li {
    display: inline-block;
    margin: 0 10px;
  }
  a {
    color: #42b983;
  }
  </style>
  